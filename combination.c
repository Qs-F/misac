...
int inc(table_t table, unsigned int key) {
  return ope(table, key, 1);
}

int dec(table_t table, unsigned int key) {
  return ope(table, key, -1);
}

void factor_reduction(table_t table, unsigned int dividend, unsigned int divisor) {
